...
#### تشخيص ملف بعينه قبل النشر
```bash
python -m scripts.ig_video_check <path-or-url>
```
يطبع حالة `ffmpeg`/`ffprobe` على الخادم، وخصائص الملف الأصلي، وأي مخالفات لمواصفات الريلز، ثم خصائص الملف **بعد** التجهيز (أي ما سيصل إلى ميتا فعلياً). مرّر عليه قيمة `mediaUrl` من أي مستند `meta_posts` فاشل.

#### ما الذي يفرضه البوت الآن قبل الرفع
يُصلَح تلقائياً بإعادة الترميز/الحاوية:
- **معدل إطارات متغيّر (VFR)** أو خارج نطاق **23–60 fps** ← يُثبَّت على `IG_REELS_TARGET_FPS` (افتراضي 30).
- **صوت ليس 48 kHz** أو أكثر من قناتين ← يُعاد ترميزه إلى **48 kHz ستيريو**.
- **مسارات زائدة** (صورة غلاف / timecode / data) ← تُحذف عبر `-map` صريح.
- **حاوية ليست MP4/MOV**، أبعاد فردية، ترميز ليس H.264، صيغة بكسل ليست yuv420p، صوت ليس AAC، أبعاد تتجاوز 1080x1920، و`mdat` قبل `moov`.

يُرفض مبكراً (لا يمكن لإعادة الترميز إصلاحه):
- مدة **أقل من 3 ثوانٍ** (`meta_err_ig_video_too_short`) أو **أكثر من 15 دقيقة** (`meta_err_ig_video_too_long`).
- نسبة أبعاد خارج **0.01:1 – 10:1** (`meta_err_ig_video_aspect_ratio`).

عند رفض ميتا للرفع رغم ذلك، يسجّل البوت **ffprobe كاملاً للبايتات المُرسَلة فعلاً** في السجلات وقناة `ERRORS_CHANNEL`، ثم يعيد المحاولة **مرة واحدة** بملف أصغر وأكثر تحفظاً (`IG_REELS_SAFE_MODE_*`) على حاوية جديدة.

> **الأهم:** بدون **ffprobe** على خادم النشر تُتخطّى كل الفحوصات أعلاه بصمت ويُرفع الملف دون تحقق — وهو أشيع سبب لـ `ProcessingFailedError`. يسجّل البوت عند الإقلاع سطراً فيه `ffprobe_available=...`؛ راجعه أولاً عند تكرار الفشل.
