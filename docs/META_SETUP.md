...
- `IG_VIDEO_STRICT_PROBE=false` (افتراضي) — إن وُضع `true`، يفشل النشر **مبكراً** إذا شغّل ffprobe لكن **لم يستطع** تحديد توافق الترميز (بدلاً من إرسال بايتات غير مؤكدة إلى ميتا ثم ظهور `ProcessingFailedError` على الرفع).
- `IG_VIDEO_CRF=18` — جودة إعادة الترميز عند الحاجة (أقل = جودة أعلى؛ 18 قريب من بلا فقدان بصري). المخرجات تبقى H.264/yuv420p/faststart كما تتطلب إنستغرام.
- `IG_VIDEO_ENCODE_PRESET=medium` — سرعة/كفاءة ترميز ffmpeg (أبطأ من `veryfast` لكن جودة أفضل عند نفس CRF).
- `IG_VIDEO_AUDIO_BITRATE=192k` — معدل بت الصوت عند إعادة ترميز الصوت إلى AAC فقط.

### إن تسرّب `pageAccessToken` (مثلاً في لقطة شاشة أو سجل)
1. في [Meta for Developers](https://developers.facebook.com/) افتح التطبيق ثم **Settings → Advanced** (أو مسارات أدوات التوكن حسب إصدار الواجهة) وراجع خيارات إبطال/تدوير التوكن إن وُجدت.
2. أعد توليد **Page access token** من Graph API Explorer أو من مسار Facebook Login مع الصلاحيات المطلوبة، ثم حدّث التوكن في تطبيق **Point** / Firestore / البوت بحيث لا يبقى التوكن المكشوف مستخدماً.
3. اعتبر أي توكن لُصق في دردشة أو تذكرة **مخترقاً** واستبدله فوراً.
...
**ملاحظة عن التخزين (R2 مقابل Supabase):** عنوان `mediaUrl` يُستخدم فقط لتحميل البايتات داخل البوت ثم رفعها إلى ميتا؛ نفس الخطأ يظهر إذا كان الفيديو مثلاً **HEVC (H.265)** داخل حاوية MP4 — إنستغرام يتوقع **H.264** في الممارسة. تأكد أن خادم تشغيل البوت يملك **ffprobe** بجانب `ffmpeg` حتى يفعّل البوت إعادة الترميز تلقائياً؛ بدون ffprobe قد يُرفع الملف كما هو فيفشل لاحقاً على ميتا.
...
## 3) جدولة النشر
عند اختيار **مستقبل** يمكن تنفيذ الجدولة بأحد أسلوبين (حسب اختيارك في واجهة الرفع):

1. **على البوت (JobQueue داخل تيليجرام)**  
   يُؤجَّل استدعاء النشر داخل البوت إلى الوقت المحدد، ثم يُنفَّذ **رفع ونشر** المحتوى في ذلك الوقت عبر Graph API (كأنك ضغت «نشر الآن» لاحقًا).

2. **على Meta (تجريبي)**  
   يُمرَّر وقت النشر إلى Graph API عبر **`scheduled_publish_time`** لنوع المنشور **فيد (feed) فقط** — إنستغرام وفيسبوك معاً أو منفصلين ضمن هذا النوع. **ريلز** و**ستوري** لا يدعمان هذا المسار في البوت؛ استخدم **على البوت** لها.  
   **فيسبوك:** عادةً يقبل ميتا أوقاتاً بين نحو **10 دقائق** و**30 يوماً** من لحظة الطلب؛ خارج ذلك قد يظهر `(#100) The specified scheduled publish time is invalid`.

**ملاحظة:** تطبيق إنستغرام و**Meta Business Suite** قد يسمحان بجدولة أنواع أخرى للمستخدم؛ القيد أعلاه لمسار **هذا البوت** فقط.
